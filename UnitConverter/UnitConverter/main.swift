...
struct UnitConvert{
    ...
    func selectBaseUnit(_ fromUnit: String) -> (Units, UnitsDic) {
        for key in UnitStruct.lengthUnitDic.keys {
            if key == fromUnit{ return (Units.cm, UnitStruct.lengthUnitDic) }
        }
        for key in UnitStruct.weightUnitDic.keys {
            if key == fromUnit { return (Units.g, UnitStruct.weightUnitDic) }
        }
        for key in UnitStruct.volumeUnitDic.keys {
            if key == fromUnit { return (Units.L, UnitStruct.volumeUnitDic) }
        }
